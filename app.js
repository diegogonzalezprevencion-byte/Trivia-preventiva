...
function isValidConsaludEmail(value) {
  return /^[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@consalud\.cl$/i.test(String(value || '').trim());
}
...
function bindAuth() {
  ...
  $('#login-form').addEventListener('submit', async event => {
    ...
    if (!isValidConsaludEmail(username)) return showAuthError('Usuario inválido. Debes utilizar un correo que termine en @consalud.cl.');
  ...
  $('#register-form').addEventListener('submit', async event => {
    ...
    if (!isValidConsaludEmail(username)) return showAuthError('Usuario inválido. Debes utilizar un correo que termine en @consalud.cl.');
    if (users.some(user => user.usernameKey === usernameKey)) return showAuthError('Ese correo corporativo ya está registrado.');
...
function renderUserDashboard() {
  ...
  $('#content').innerHTML = `
    <section class="hero-banner">
      <div><p class="eyebrow">Desafío mensual</p><h3>${completed ? '¡Trivia de hoy completada!' : 'Tu decisión segura comienza hoy'}</h3><p>${completed ? 'Mañana tendrás una nueva situación preventiva.' : 'Responde la pregunta diaria, aprende con la explicación y suma una Estrella cuando aciertes.'}</p><button class="btn" id="hero-action">${completed ? 'Ver mi historial' : 'Responder trivia diaria'}</button></div>
    ...
    <section class="grid stats">
      ...
      ${statCard('Estrellas', ticketCount(monthly), '★')}
    </section>
    <section class="grid two">
      <article class="card instructions-card">
        ...
        <div class="category-list">
          ...
          <div class="category-row"><span class="rank-pill">3</span><div><strong>Aprende y participa</strong><span>Al acertar sumas una Estrella para el sorteo.</span></div></div>
        </div>
      </article>
...
function renderTrivia() {
  ...
  if (!question) {
    $('#content').innerHTML = `<div class="trivia-wrap"><article class="question-card completed-state"><div class="big-icon">📅</div><h3>No hay una pregunta programada para hoy</h3><p>El banco inicial contiene 600 preguntas diarias desde el 1 de enero de 2026. El administrador puede revisar, modificar o agregar nuevas preguntas.</p></article></div>`;
    return;
  }
  selectedAnswer = null;
  const meta = CATEGORY_META[question.category];
  $('#content').innerHTML = `<div class="trivia-wrap"><div class="trivia-top"><span class="category-badge prominent">${meta.icon} ${meta.name}</span><span class="category-badge">Pregunta ${question.sequence} de ${load(KEYS.questions).length} · ${formatDate(question.scheduleDate)}</span></div><article class="question-card"><p class="eyebrow">Situación del día</p><h3>${escapeHtml(question.question)}</h3><div class="answers">${question.options.map((option, index) => `<button class="answer" data-answer="${index}"><span class="answer-key">${String.fromCharCode(65 + index)}</span><span>${escapeHtml(option)}</span></button>`).join('')}</div><button id="submit-answer" class="btn primary full" style="margin-top:20px" disabled>Confirmar respuesta</button><div id="feedback-area"></div></article></div>`;
...
function submitAnswer(question) {
  ...
  $('#feedback-area').innerHTML = `<div class="feedback ${isCorrect ? 'success' : 'error'}"><h4>${isCorrect ? '¡Respuesta correcta! Sumaste una Estrella.' : 'Respuesta incorrecta. Hoy no sumaste una Estrella.'}</h4><p>${escapeHtml(question.explanation)}</p><button id="finish-trivia" class="btn secondary">Volver al inicio</button></div>`;
...
function renderAdminDashboard() {
  const allQuestions = load(KEYS.questions);
  const questionDates = allQuestions.map(question => question.scheduleDate).filter(Boolean).sort();
  const bankEndDate = questionDates.length ? questionDates[questionDates.length - 1] : defaultSettings.questionBankStart;
  ...
  $('#content').innerHTML = `
    ...
    <section class="grid two admin-summary-grid">
      ...
      <article class="card"><h3>Resumen del banco</h3><p class="card-subtitle">Programación preventiva disponible.</p><div class="bank-summary"><strong>${allQuestions.length}</strong><span>preguntas programadas</span><small>Desde 01-01-2026 hasta ${formatDate(bankEndDate)}</small></div></article>
    </section>
    <article class="card admin-section"><div class="toolbar"><div><h3>Ranking por preguntas respondidas</h3><p class="card-subtitle">Orden mensual de mayor a menor participación.</p></div><button class="btn secondary" id="export-dashboard-ranking">Descargar Excel</button></div><div class="table-wrap"><table><thead><tr><th>Posición</th><th>Correo corporativo</th><th>Sucursal</th><th>Respondidas</th><th>Correctas</th><th>Incorrectas</th><th>Efectividad</th><th>Estrellas</th></tr></thead><tbody>${ranking.length ? ranking.map((row, index) => `<tr><td><span class="rank-pill ${index < 3 ? 'top' : ''}">${index + 1}</span></td><td><b>${escapeHtml(row.username)}</b></td><td>${escapeHtml(row.branch)}</td><td><b>${row.total}</b></td><td>${row.correct}</td><td>${row.incorrect}</td><td>${row.rate}%</td><td>${row.tickets}</td></tr>`).join('') : '<tr><td colspan="8" class="empty">No existen respuestas en el periodo seleccionado.</td></tr>'}</tbody></table></div></article>`;
  ...
  $('#export-dashboard-ranking').addEventListener('click', () => exportRowsAsExcel(ranking.map((row, index) => ({ Posición:index+1, Usuario:row.username, Sucursal:row.branch, Respondidas:row.total, Correctas:row.correct, Incorrectas:row.incorrect, Efectividad:`${row.rate}%`, Estrellas:row.tickets, Periodo:periodLabel(adminPeriod.year, adminPeriod.month) })), `ranking-participacion-${periodKey(adminPeriod.year, adminPeriod.month)}`));
}
...
function renderAdminManagement() {
  ...
  $('#content').innerHTML = `
    <article class="card admin-section"><div class="toolbar"><div><h3>Personas registradas</h3><p class="card-subtitle">Gestión de correo corporativo, sucursal, restablecimiento de clave y eliminación de cuentas. Por seguridad, las claves existentes permanecen cifradas y no se muestran.</p></div><button class="btn secondary" id="export-users">Descargar registro</button></div>
      ${editingUser ? `<form id="user-edit-form" class="editor-panel"><div class="editor-header"><div><p class="eyebrow">Editando cuenta</p><h3>${escapeHtml(editingUser.username)}</h3></div><button type="button" class="btn ghost" id="cancel-user-edit">Cancelar</button></div><div class="form-grid"><label>Correo corporativo<input id="edit-username" type="text" inputmode="email" value="${escapeHtml(editingUser.username)}" placeholder="nombre.apellido@consalud.cl" required><span class="field-hint">Debe terminar en @consalud.cl.</span></label><label>Sucursal<input id="edit-branch" value="${escapeHtml(editingUser.branch)}" required></label><label class="span-2">Nueva clave opcional<input id="edit-password" type="password" placeholder="Dejar vacío para conservar la clave actual"><span class="field-hint">Mínimo 6 caracteres, una mayúscula y un número.</span></label></div><button class="btn primary" type="submit">Guardar cambios</button></form>` : ''}
      <div class="table-wrap"><table><thead><tr><th>Correo corporativo</th><th>Sucursal</th><th>Fecha de registro</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${users.length ? users.map(user => `<tr><td><b>${escapeHtml(user.username)}</b>${!isValidConsaludEmail(user.username) ? '<br><span class="status invalid">Usuario inválido</span>' : ''}</td><td>${escapeHtml(user.branch)}</td><td>${new Intl.DateTimeFormat('es-CL', { dateStyle:'medium' }).format(new Date(user.createdAt))}</td><td><span class="status protected">Clave protegida</span></td><td><div class="toolbar-actions"><button class="btn secondary edit-user" data-id="${user.id}">Editar</button><button class="btn danger delete-user" data-id="${user.id}" data-name="${escapeHtml(user.username)}">Eliminar</button></div></td></tr>`).join('') : '<tr><td colspan="5" class="empty">Todavía no hay personas registradas.</td></tr>'}</tbody></table></div>
    </article>
    <article class="card admin-section"><div class="toolbar"><div><h3>Banco de 600 preguntas</h3><p class="card-subtitle">Banco base de 600 preguntas programadas. Puedes buscar, editar y agregar nuevas preguntas cuando sea necesario.</p></div><div class="toolbar-actions"><button class="btn primary" id="add-question">+ Agregar pregunta</button><button class="btn ghost" id="export-questions">Descargar banco</button></div></div>
      ${creatingQuestion ? renderQuestionCreator(questions) : ''}
      ...
      <p class="result-count">${filteredQuestions.length} pregunta(s) encontrada(s) · ${questions.length} pregunta(s) en el banco.</p>
  ...
  $$('.delete-user').forEach(button => button.addEventListener('click', () => deleteUser(button.dataset.id, button.dataset.name)));
  ...
  $('#export-users').addEventListener('click', () => exportRowsAsExcel(users.map(user => ({ Correo_corporativo:user.username, Sucursal:user.branch, Fecha_de_registro:new Intl.DateTimeFormat('es-CL').format(new Date(user.createdAt)), Estado:isValidConsaludEmail(user.username) ? 'Válido' : 'Usuario inválido', Clave:'Protegida - restablecimiento disponible' })), 'usuarios-trivia'));
  ...
  $('#cancel-question-create')?.addEventListener('click', () => { creatingQuestion = false; renderAdminManagement(); });
  $('#question-create-form')?.addEventListener('submit', saveNewQuestion);
  ...
  $$('.edit-question').forEach(button => button.addEventListener('click', () => { editingQuestionId = button.dataset.id; creatingQuestion = false; renderAdminManagement(); window.scrollTo({ top: 0, behavior:'smooth' }); }));
...
function nextQuestionDate(questions) {
  const dates = questions.map(question => question.scheduleDate).filter(Boolean).sort();
  return dates.length ? addDays(dates[dates.length - 1], 1) : defaultSettings.questionBankStart;
}

function renderQuestionCreator(questions) {
  const date = nextQuestionDate(questions);
  return `<form id="question-create-form" class="editor-panel"><div class="editor-header"><div><p class="eyebrow">Nueva pregunta</p><h3>Agregar al banco</h3></div><button type="button" class="btn ghost" id="cancel-question-create">Cancelar</button></div><div class="form-grid"><label>Fecha programada<input id="new-q-date" type="date" value="${date}" required></label><label>Categoría<select id="new-q-category">${Object.entries(CATEGORY_META).map(([key, meta]) => `<option value="${key}">${meta.name}</option>`).join('')}</select></label><label class="span-2">Pregunta<textarea id="new-q-text" required></textarea></label><label>Alternativa A<input id="new-q-a" required></label><label>Alternativa B<input id="new-q-b" required></label><label>Alternativa C<input id="new-q-c" required></label><label>Alternativa correcta<select id="new-q-correct"><option value="0">A</option><option value="1">B</option><option value="2">C</option></select></label><label class="span-2">Explicación<textarea id="new-q-explanation" required></textarea></label></div><button class="btn primary" type="submit">Agregar pregunta</button></form>`;
}

function deleteUser(userId, username) {
  const confirmed = window.confirm(`¿Eliminar a ${username}? Esta acción también eliminará sus respuestas y Estrellas acumuladas. El historial de sorteos ya realizados se conservará.`);
  if (!confirmed) return;
  const users = load(KEYS.users).filter(user => user.id !== userId);
  const attempts = load(KEYS.attempts).filter(attempt => attempt.userId !== userId);
  save(KEYS.users, users);
  save(KEYS.attempts, attempts);
  if (editingUserId === userId) editingUserId = null;
  toast('Usuario y sus registros de participación fueron eliminados.');
  renderAdminManagement();
}
...
function saveNewQuestion(event) {
  event.preventDefault();
  const questions = load(KEYS.questions);
  const date = $('#new-q-date').value;
  const text = $('#new-q-text').value.trim();
  const options = [$('#new-q-a').value.trim(), $('#new-q-b').value.trim(), $('#new-q-c').value.trim()];
  const explanation = $('#new-q-explanation').value.trim();
  if (questions.some(question => question.scheduleDate === date)) return toast('Ya existe una pregunta programada para esa fecha.');
  if (!text || options.some(option => !option) || !explanation) return toast('Completa todos los campos de la pregunta.');
  if (new Set(options.map(option => option.toLocaleLowerCase('es-CL'))).size !== 3) return toast('Las tres alternativas deben ser diferentes.');
  const sequence = questions.reduce((max, question) => Math.max(max, Number(question.sequence) || 0), 0) + 1;
  questions.push({
    id: `q-${crypto.randomUUID()}`,
    sequence,
    scheduleDate: date,
    category: $('#new-q-category').value,
    question: text,
    options,
    correct: Number($('#new-q-correct').value),
    explanation,
    createdAt: new Date().toISOString(),
    updatedAt: null
  });
  ...
  creatingQuestion = false;
  questionFilters = { day: date, month:'', year:'', search:'' };
  toast('Nueva pregunta agregada correctamente.');
  renderAdminManagement();
}
...
async function saveUserChanges(event) {
  ...
  if (!isValidConsaludEmail(username)) return toast('Usuario inválido. Debe utilizar un correo que termine en @consalud.cl.');
  if (users.some(item => item.id !== user.id && item.usernameKey === key)) return toast('Ese correo corporativo ya está registrado.');
...
function renderDraw() {
  ...
  $('#content').innerHTML = `
    ...
    <section class="grid draw-stats">${statCard('Participantes del periodo', participants.length, '👥')}${statCard('Máximo de Estrellas', maxTickets, '★')}${statCard('Personas habilitadas', eligible.length, '🏆')}${statCard('Premio', money(settings.prizeAmount), '🎁')}</section>
    <article class="card draw-highlight"><div><p class="eyebrow">Regla del sorteo</p><h3>Participan únicamente quienes alcanzaron el máximo de Estrellas</h3><p>${eligible.length ? `En este periodo, ${eligible.length} persona(s) tienen ${maxTickets} Estrella(s) y forman el grupo habilitado.` : 'No existen participantes con Estrellas en el periodo seleccionado.'}</p></div><button id="run-period-draw" class="btn primary" ${!eligible.length || existingDraw ? 'disabled' : ''}>${existingDraw ? 'Sorteo ya realizado' : 'Realizar sorteo'}</button></article>
    ...
    <article class="card admin-section"><div class="toolbar"><div><h3>Lista de participantes</h3><p class="card-subtitle">El distintivo “Habilitado” identifica a quienes tienen el máximo de Estrellas.</p></div><button class="btn secondary" id="export-draw-list">Descargar Excel</button></div><div class="table-wrap"><table><thead><tr><th>Correo corporativo</th><th>Sucursal</th><th>Respondidas</th><th>Correctas</th><th>Incorrectas</th><th>Estrellas</th><th>Estado</th></tr></thead><tbody>${participants.length ? participants.map(row => `<tr class="${row.tickets === maxTickets && maxTickets > 0 ? 'eligible-row' : ''}"><td><b>${escapeHtml(row.username)}</b></td><td>${escapeHtml(row.branch)}</td><td>${row.total}</td><td>${row.correct}</td><td>${row.incorrect}</td><td><b>${row.tickets}</b></td><td>${row.tickets === maxTickets && maxTickets > 0 ? '<span class="status eligible">Habilitado</span>' : '<span class="status neutral">No habilitado</span>'}</td></tr>`).join('') : '<tr><td colspan="7" class="empty">No existen participantes en el periodo seleccionado.</td></tr>'}</tbody></table></div></article>
    <article class="card admin-section"><h3>Historial de sorteos</h3><p class="card-subtitle">Registro de los periodos sorteados.</p><div class="table-wrap"><table><thead><tr><th>Fecha del sorteo</th><th>Periodo</th><th>Ganador</th><th>Sucursal</th><th>Máximo de Estrellas</th><th>Habilitados</th><th>Premio</th></tr></thead><tbody>${draws.length ? draws.slice().sort((a,b) => b.createdAt.localeCompare(a.createdAt)).map(draw => `<tr><td>${new Intl.DateTimeFormat('es-CL', { dateStyle:'medium', timeStyle:'short' }).format(new Date(draw.createdAt))}</td><td>${escapeHtml(draw.periodLabel)}</td><td><b>${escapeHtml(draw.winnerName)}</b></td><td>${escapeHtml(draw.branch)}</td><td>${draw.maxTickets}</td><td>${draw.eligibleCount}</td><td>${money(draw.prizeAmount)}</td></tr>`).join('') : '<tr><td colspan="7" class="empty">Todavía no se han realizado sorteos.</td></tr>'}</tbody></table></div></article>`;
  ...
  $('#export-draw-list').addEventListener('click', () => exportRowsAsExcel(participants.map(row => ({ Usuario:row.username, Sucursal:row.branch, Respondidas:row.total, Correctas:row.correct, Incorrectas:row.incorrect, Estrellas:row.tickets, Estado:row.tickets === maxTickets && maxTickets > 0 ? 'Habilitado' : 'No habilitado', Periodo:periodLabel(drawPeriod.year, drawPeriod.month) })), `participantes-sorteo-${periodKey(drawPeriod.year, drawPeriod.month)}`));
}
...
function restoreSession() {
  ...
  if (user?.role === 'user' && !isValidConsaludEmail(user.username)) {
    localStorage.removeItem(KEYS.session);
    return;
  }
