...
function bindAuth() {
  ...
  $('#register-form').addEventListener('submit', async event => {
    ...
    try {
      ...
      if (error) throw error;
      if (!data.session) {
        showAuthError('La cuenta fue creada, pero Supabase todavía exige confirmación por correo. Desactiva “Confirm email” para permitir el ingreso inmediato.', 'error');
        return;
      }
      $('#register-form').reset();
      await enterApp(data.user, 'user');
      toast('Cuenta creada. Tu correo corporativo quedó registrado como respaldo; no necesitas confirmarlo.');
    } catch (error) { showAuthError(readableError(error)); }
  });
