...
function readableError(error, fallback = 'Ocurrió un problema. Intenta nuevamente.') {
  ...
  if (/Email not confirmed/i.test(message)) return 'La confirmación de correo sigue activa en Supabase. Debe desactivarse para permitir el ingreso inmediato.';
...
function bindAuth() {
  ...
  $('#register-form').addEventListener('submit', async event => {
    ...
    try {
      ...
      // Algunos proyectos de Supabase no devuelven una sesión en el mismo
      // registro, aunque la cuenta ya haya sido confirmada por la regla de
      // la base de datos. En ese caso iniciamos sesión inmediatamente.
      ...
      $('#register-form').reset();
      await enterApp(registeredUser, 'user');
      toast('Cuenta creada correctamente. Tu correo corporativo quedó registrado como respaldo.');
    } catch (error) { showAuthError(readableError(error)); }
  });
...
async function renderHistory() {
  renderLoading();
  try {
    const { data: attempts, error } = await sb.rpc('get_my_history_details');
    if (error) throw error;
    const rows = attempts || [];
    $('#content').innerHTML = `<article class="card admin-section"><h3>Mi historial completo</h3><p class="card-subtitle">Aquí puedes revisar la pregunta, la alternativa que elegiste y el resultado obtenido. El historial se conserva aunque los indicadores mensuales vuelvan a cero.</p><div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Pregunta</th><th>Alternativa elegida</th><th>Resultado</th><th>Estrellas</th></tr></thead><tbody>${rows.length ? rows.map(a => `<tr><td>${formatDate(a.answered_on)}</td><td class="history-question">${escapeHtml(a.question_text)}</td><td class="history-answer"><strong>${escapeHtml(a.answer_letter)}</strong>${escapeHtml(a.selected_option)}</td><td>${a.is_correct ? '<span class="status eligible">Correcta</span>' : '<span class="status neutral">Incorrecta</span>'}</td><td><b>${a.stars}</b></td></tr>`).join('') : '<tr><td colspan="5" class="empty">Todavía no has respondido preguntas.</td></tr>'}</tbody></table></div></article>`;
  } catch (error) {
    $('#content').innerHTML = `<article class="card"><h3>No se pudo cargar el historial detallado</h3><p>${escapeHtml(readableError(error))}</p></article>`;
  }
}

function renderPeriodFilters(prefix, period) {
  return `<div class="analysis-controls"><label><span>Mes</span><select id="${prefix}-month">${MONTHS.map((m,i) => `<option value="${i+1}" ${period.month === i+1 ? 'selected' : ''}>${m}</option>`).join('')}</select></label><label><span>Año</span><select id="${prefix}-year">${Array.from({length:75},(_,i)=>2026+i).map(y => `<option value="${y}" ${period.year === y ? 'selected' : ''}>${y}</option>`).join('')}</select></label><button class="btn primary analysis-apply" id="${prefix}-apply">Aplicar filtros</button></div>`;
}
